...
def create_local_map(glo_map,current_x, current_y):
    loc_map = np.ones((40,40))
    map = np.copy(glo_map)
    x=current_x*2
    y=map.shape[0]-current_y*2
    start_x = max(int(x) - 20, 0)
    start_y = max(int(y) - 20, 0)
    end_x = min(start_x + 40, map.shape[1])
    end_y = min(start_y + 40, map.shape[0])
...
def transformation_matrix(center,arr_point_in):
    ...
    rotation_matrix = np.array([[1, 0 ],
                                [0, -1],])
    translation_vector = np.array([center[0]*2-20, 140-(center[1]*2-20)])
...
globalMap = cv2.imread('global_map_parking.png', cv2.IMREAD_GRAYSCALE)
...
def table_info():
  ...
  # hien thi ket qua khi an nut set
    ...
    def set_click():
        ...
        y_goal=float(y_goal)
        a_goal=float(a_goal)
        goalPoseGlobal = [x_goal*2,y_goal*2,(a_goal+90)*pi/180]

        #tạo quỹ đạo di chuyển trên global map
        start_state = [x_init*2, y_init*2, (actual_angle+90)*pi/180]
        
        list_state_global,_ = calculate_trajectory(map = init_map ,startPoint = start_state, goalPoint = goalPoseGlobal,space= 1)
...
def car_auto_control():
    ...
    while run:
        ...
        x_1,y_1 = calulation_next_point(start_point_X= x_0,start_point_Y=y_0,distance_turn = actual_dis-last_actual_dis,angle_turn=actual_angle)
        x_0,y_0 = x_1,y_1
        last_actual_dis = actual_dis
        ...
        # print(goalChecked)
        s_state = [20, 20, (actual_angle+90)*pi/180]
        if check_colision == False:
            list_state, list_direction = calculate_trajectory(map = chk_local_map ,startPoint = s_state, goalPoint = goalChecked, space=1)
            draw_signal = True
